import { db, queryClient } from '../lib/server/db/index';
...
const seed = async () => {
	...
	await db.transaction(async (tx) => {
		...
		await tx.delete(schema.meaningRelation);
		await tx.delete(schema.translation);
		await tx.delete(schema.wordMeaning);
		await tx.delete(schema.word);

		// 2. Insert Words
		console.log('Inserting words...');
		const wordsData: (typeof schema.word.$inferInsert)[] = [
			// English Verbs
			{ langPair: 'en_es', word: 'run', lang: 'en', pos: 'verb' },
			{ langPair: 'en_es', word: 'manage', lang: 'en', pos: 'verb' },
			{ langPair: 'en_es', word: 'direct', lang: 'en', pos: 'verb' },
			{ langPair: 'en_es', word: 'execute', lang: 'en', pos: 'verb' },
			{ langPair: 'en_es', word: 'flow', lang: 'en', pos: 'verb' },
			// English Nouns
			{ langPair: 'en_es', word: 'run', lang: 'en', pos: 'noun' },

			// Spanish Verbs
			{ langPair: 'en_es', word: 'correr', lang: 'es', pos: 'verb' },
			{ langPair: 'en_es', word: 'dirigir', lang: 'es', pos: 'verb' },
			{ langPair: 'en_es', word: 'ejecutar', lang: 'es', pos: 'verb' },
			{ langPair: 'en_es', word: 'fluir', lang: 'es', pos: 'verb' },
			// Spanish Nouns
			{ langPair: 'en_es', word: 'carrera', lang: 'es', pos: 'noun' }
		];

		const wordMap = new Map<string, number>();
		const insertedWords = await tx.insert(schema.word).values(wordsData).returning();
		for (let i = 0; i < wordsData.length; i++) {
			const w = wordsData[i];
			wordMap.set(`${w.word}:${w.pos}`, insertedWords[i].id);
		}
		...
		const meaningsData = [
			...
			// carrera (noun)
			{
				key: 'carrera:noun_act',
				wordKey: 'carrera:noun',
		...
		const meaningsInsertValues = meaningsData.map((m) => {
		...
		const insertedMeanings = await tx.insert(schema.wordMeaning).values(meaningsInsertValues).returning();
		for (let i = 0; i < meaningsData.length; i++) {
			meaningMap.set(meaningsData[i].key, insertedMeanings[i].id);
		}
		...
		const translationInsertValues = translationsData.map((t) => {
			...
			return { langPair: 'en_es', srcMeaningId: srcId, dstMeaningId: dstId } as const;
		});

		await tx.insert(schema.translation).values(translationInsertValues);
	});
